...
class SelectQueryTest extends TestCase
{
    ...
    public function testSelectWithSingleWhere()
    {
        $query = new QueryBuilder();
    ...
    public function testSelectWithWhereAlternateSyntax()
    {
        $query = new QueryBuilder();
        $query->select('column1', 'column2', 'column3')
            ->from('table1 t1')
            ->where('t1.id = ?', 'OR', '(t1.email = ? AND t1.username = ?)', 'AND', 't1.created_at < ?');
            $this->assertEquals('SELECT column1,column2,column3 FROM table1 t1 WHERE t1.id = ? OR (t1.email = ? AND t1.username = ?) AND t1.created_at < ?', (string) $query);
    }
    ...
    public function testSelectWithInnerJoin()
    {
        ...
        $this->assertEquals('SELECT column1,column2,column3 FROM table1 t1 INNER JOIN table2 t2 ON t1.id = t2.id', (string) $query);
    }
    ...
    public function testSelectWithMultipleJoins()
    {
        ...
        $this->assertEquals('SELECT column1,column2,column3 FROM table1 t1 INNER JOIN table2 t2 ON t1.id = t2.id LEFT JOIN table3 t3 ON t1.id = t3.id', (string) $query);
    }
